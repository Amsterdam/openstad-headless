...
const privateKey = process.env.PRIVATE_KEY ? Buffer.from(process.env.PRIVATE_KEY, 'base64').toString('utf8') : fs.readFileSync(path.join(__dirname, certPath + '/privatekey.pem'));

/** Public certificate used for verification.  Note: you could also use the private key */
const publicKey = process.env.PUBLIC_KEY ? Buffer.from(process.env.PUBLIC_KEY, 'base64').toString('utf8') : fs.readFileSync(path.join(__dirname, certPath + '/certificate.pem'));
