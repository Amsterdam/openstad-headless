...
const privateKey = process.env.PRIVATE_KEY ?? fs.readFileSync(path.join(__dirname, certPath + '/privatekey.pem'));

/** Public certificate used for verification.  Note: you could also use the private key */
const publicKey = process.env.PUBLIC_KEY ?? fs.readFileSync(path.join(__dirname, certPath + '/certificate.pem'));
